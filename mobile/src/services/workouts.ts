...
export async function updateWorkout(workout: Workout): Promise<Workout> {
  ...
  const { data: exerciseRows, error: exerciseError } = await supabase
    ...
    .insert(workout.exercises.map((exercise, position) => ({ exercise_id: exercise.exerciseId ?? null, exercise_name: exercise.name, position, workout_id: workout.id })))
    .select('id');
...
export const formatWorkoutDate = (value: string) => new Intl.DateTimeFormat(undefined, { day: 'numeric', month: 'short', year: 'numeric' }).format(new Date(value));
