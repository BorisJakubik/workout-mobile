...
export function WorkoutCalendar({ onSelectWorkout, workouts }: WorkoutCalendarProps) {
  ...
  const title = new Intl.DateTimeFormat(language === "sk" ? "sk-SK" : undefined, { month: "long", year: "numeric" }).format(visibleMonth);
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.weekdays}>
        {(language === "sk" ? ["P", "U", "S", "Š", "P", "S", "N"] : ["M", "T", "W", "T", "F", "S", "S"]).map((day, index) => (
      ...
      <Text style={styles.selectedTitle}>
        {new Intl.DateTimeFormat(language === "sk" ? "sk-SK" : undefined, { dateStyle: "long" }).format(new Date(`${selectedDate}T12:00:00`))}
      </Text>
      {selectedWorkouts.length ? (
        selectedWorkouts.map((workout) => (
          <Pressable key={workout.id} onPress={() => onSelectWorkout(workout)} style={styles.result}>
            <Text style={styles.resultName}>{translateWorkoutName(workout.name, language)}</Text>
            <Text style={styles.resultMeta}>
              {workout.duration} min · {workout.exercises.length} {translate(language, "exercises")}
            </Text>
          </Pressable>
        ))
      ) : (
        <Text style={styles.noResult}>
          {language === "sk" ? "V tento deň nebol zaznamenaný žiadny tréning." : "No workout recorded on this day."}
        </Text>
      )}
    </View>
  );
